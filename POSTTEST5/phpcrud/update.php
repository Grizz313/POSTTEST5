...
if (isset($_GET['id'])) {
    if (!empty($_POST)) {
        ...
        $pekerjaan = isset($_POST['biografi']) ? $_POST['biografi'] : '';
        
        // Update the record
        $stmt = $pdo->prepare('UPDATE kontak SET id = ?, nama = ?, email = ?, notelp = ?, biografi = ? WHERE id = ?');
        $stmt->execute([$id, $nama, $email, $notelp, $biografi, $_GET['id']]);
...
<div class="content update">
    ...
    <form action="update.php?id=<?=$contact['id']?>" method="post">
        ...
        <label for="biografi">ABOUT YOURSELF</label>
        <input type="text" name="biografi" value="<?=$contact['biografi']?>" id="title">
